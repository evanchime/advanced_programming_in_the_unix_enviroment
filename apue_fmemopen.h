#ifndef _APUE_FMEMOPEN_H
#define _APUE_FMEMOPEN_H
...
mstream_read(void *cookie, char *buf, int len){ 
	...
	if (ms->curpos >= ms->vsize) 
		return(0); /* can only read from curpos to vsize */ 
